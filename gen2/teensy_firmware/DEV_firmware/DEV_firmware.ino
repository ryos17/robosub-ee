...
//   * The kill ISR neutralizes all actuators immediately on assertion (no
//     wait for the main loop), then the firmware blocks in the kill island:
//     all actuators held neutral, UART drained, periodic "[KILLED]" message
//     to the Orin.
// ============================================================================
...
inline int clampThrusterUS(int us) {
  if (us < THRUSTER_MIN_US) {
    return THRUSTER_MIN_US;
  }
  if (us > THRUSTER_MAX_US) {
    return THRUSTER_MAX_US;
  }
  return us;
}
...
volatile bool isKilled = false;                       // set by the ISR, the instability watchdog, and the kill island; seeded from the pin at boot
...
const int sdLoggingFrequency = 20000;    // log every N loop passes
...
void config_depth() {
  ...
  if (!depthSensorOk) {
    Serial.println("[DEPTH] MS5837 init FAILED -- depth telemetry will read 0");
  }
...
void killISR() {
  ...
  if (nowUs - _lastKillIsrUs < KILL_ISR_DEBOUNCE_US) {
    return;
  }
  ...
  // Neutralize every actuator RIGHT HERE in the ISR, not just in the kill
  // island. writeMicroseconds() only updates timer registers, so it is safe
  // to call from an interrupt on Teensy. This removes the one-loop-pass kill
  // latency (worst case ~40 ms during a blocking MS5837 read) and, if the
  // main loop is ever wedged (e.g. I2C bus hang), the thrusters still stop
  // the instant the switch is pulled even though the island can't run.
  if (asserted) {
    applyNeutralAll();
  }
...
// Put every actuator in its safe state: all 8 thrusters to neutral (1500 us),
// dropper parked at 14 deg (its holding position -- NOT the 0 deg boot pose,
// intentional), torpedo at 0 deg.
// Called from the kill ISR as well as the kill island, so it must stay
// ISR-safe: writeMicroseconds() only (timer registers), no Serial, no heap.
void applyNeutralAll() {
  for (int i = 0; i < 8; i++) {
...
void loop() {
  // The kill switch is checked FIRST, every pass. On assertion the ISR has
  // already neutralized the actuators; here we drop into the kill island
  // and do not come back until the switch is re-enabled.
...
void updateInstabilityWatchdog() {
  ...
  unsigned long now = millis();
  if (now - windowStartMs < INSTABILITY_WINDOW_MS) {
    return;
  }
...
void updateRedLed(bool heartbeatMonitored) {
  ...
  if (now - lastBatteryCheckMs >= BATTERY_CHECK_PERIOD_MS) {
    ...
    if (lowBattery) {
      if (voltage > LOW_BATTERY_VOLTAGE + LOW_BATTERY_HYSTERESIS) {
        lowBattery = false;
      }
...
// ========================= KILL ISLAND =========================
// A hard, blocking loop entered from loop() when the kill switch is asserted
// (the kill ISR has already neutralized every actuator by this point). Until
// the switch is physically released this does ONLY two things:
...
void enterKillIsland() {
  ...
  noInterrupts();
  killEdgeCount = 0;
  interrupts();  // ignore edges from before entry
#endif
  ...
  noInterrupts();
  killEdgeCount = 0;
  interrupts();  // fresh window for normal operation
#endif
  ...
  bufferPosition = 0;
  while (Serial.available() > 0) {
    (void)Serial.read();  // start with an empty buffer
  }
...
#define HB_ACK_REPLY 0
  ...
  if (sscanf(input, "%d %d %d %d %d %d %d %d %n",
  ...
  } else if (sscanf(input, "%d %d %n", &servoNum, &val, &consumed) == 2 && input[consumed] == '\0') {  // Set single thruster (exact parse; rejects values outside THRUSTER_MIN/MAX)
    ...
    if (!depthSensorOk) {
      Serial.println("MS5837 init failed at boot -- values are stale zeros");
    }
    ...
    // Clamp for safety
    if (dropperDeg < -dropperHalfRangeDeg) {
      dropperDeg = -dropperHalfRangeDeg;
    }
    if (dropperDeg > +dropperHalfRangeDeg) {
      dropperDeg = +dropperHalfRangeDeg;
    }
...
// BENCH USE ONLY: blocks ~20 s in delay() with no kill check. Pulling the
// kill switch mid-test neutralizes instantly (ISR), but this loop then
// re-commands the NEXT thruster at 1550 until it finishes.
void test_servos() {
  for (int i = 0; i < 8; i++) {
